...
- Rival Counter-Doctrine: a nearby rival governor now forms explainable counter-lessons only from Crown actions inside shared enemy vision. Observed assaults strengthen defense, fixed lines invite an expedition, visible caravans invite logistics interdiction, and observed mobilization encourages recruitment. The same doctrine and event systems expose every adaptation without hidden vision or stat bonuses.
- Rival Intelligence: the Book of Lessons now retains the two newest visible enemy counter-doctrines, while the Tactical Uplink briefly elevates a fresh enemy adaptation when no higher-priority civic or military crisis exists. This uses a pure projection of authoritative doctrine state and adds no new tactical panel or map obstruction.
- Fog-respecting Reconnaissance Contacts: left-clicking an already visible rival battalion, structure, or convoy now gives the existing Tactical Uplink a concise condition and terrain briefing, with a blue map focus that preserves Crown selection for the next order. Hidden contacts return no intel and no new management surface is introduced.
- Compact Terrain Badges: map zones retain their terrain symbol, name, and core use while their full rule detail stays in existing placement and theatre guidance, protecting tactical visibility and normal map input.
- Field Camera Recovery: Command surfaces overlay the authoritative battlefield rather than reserving an empty desktop strip, so Field view clears the HUD to a full map camera with the existing pan, zoom, and minimap controls.
