...
| `I` / `UPLINK` | Toggle the Tactical Uplink's live operation brief. Every tactical reign begins with a compact Mandate-progress tab to preserve upper-left battlefield space; the complete report opens only on demand. This is presentation-only and `Esc` closes it with other temporary surfaces. |
...
Every interactive child of a fixed Phaser HUD or panel must independently use a zero scroll factor. Camera bounds and world scrolling may never displace the hit area of a visibly screen-anchored tactical, campaign, management, minimap, or victory control. A tactical child that is hidden must also have pointer eligibility disabled; visual invisibility alone is never permitted to reserve a dead battlefield region. World selection is presentation-derived only: selected Crown battalions and convoys receive a high-contrast green halo, the active Crown Castle receives the same command-seat outline, and clicked visible rival contacts retain a distinct blue reconnaissance focus. These affordances create no command, state mutation, save, replay, or network message.

The tactical camera must remain usable while management surfaces are open. Arrow-key panning, cursor-anchored wheel zoom, and middle-button map drag are always available over the battlefield. A 28px clear gutter on every tactical screen edge provides smoothly accelerating map-edge scrolling in both Command and Field presentation, avoiding interaction with the screen-anchored controls; the minimap accepts click-and-drag camera scrubbing as well as jump clicks. Camera bounds always match the authoritative map: management surfaces overlay the field in Command view rather than reserving an empty strip, and Field view therefore reveals the entire current camera view. The Orders dock begins as a compact header and the Accord, Heir, and Build headers form a narrow right-hand rail; expanding a single drawer hides the inactive tabs, so at most one temporary management surface covers the battlefield. Decorative panel backgrounds and readouts must never capture pointer input: only explicit headers, buttons, tiles, modal surfaces, and the minimap may own a gesture, leaving unused drawer space available to normal battlefield selection, placement, and orders. Middle-button gestures on the right-hand rail never activate a management action. Camera movement is renderer-only and must never create commands, modify selection, or enter saves, replays, or multiplayer authority. A selected group receives a presentation-derived, ID-stable line formation perpendicular to its direction of travel; the scene turns those destinations into ordinary individual commands, so formation marching is replay-safe and needs no simulation special case. Shift-queued routes are likewise ordinary authoritative movement commands, serialized in the battalion state, included in deterministic saves, replays, snapshots, and hashes, and rendered as one visible leg per waypoint. `hold-battalion` is the direct `V` stance: it clears a battalion's route and target, keeps it at its current ground, permits only in-range defensive fire, and excludes it from governor reassignment until a later move, advance, attack, retreat, embarkation, or garrison order clears the stance.
...
- `rendering/rivalIntelligencePresentation.ts` is the pure presentation projection for durable enemy adaptation. It returns only living Rival Crown heirs' counter-doctrines, ordered by authoritative update tick, confidence, then ID. The Book of Lessons presents the two latest entries with observer, action, and confidence; when no witnessed action exists it says so explicitly. The Tactical Uplink threat forecast may surface the newest counter-doctrine for eighteen ticks after it is formed, but immediate food, rebellion, religious, and castle-loss warnings retain priority. This presentation never reveals unseen state, queues commands, or mutates simulation, saves, replays, or multiplayer authority.
- `rendering/reconnaissancePresentation.ts` is the pure, fog-respecting contact projection for a deliberately clicked rival battalion, structure, or convoy. It returns nothing for a Crown asset or a contact outside normal Crown visibility. A visible contact may state its public force/defense condition and current terrain consequence in the existing Tactical Uplink; inspection retains the player’s field selection, gains only a blue presentation focus, and clears immediately when the contact is hidden, destroyed, or the player clears selection. This does not create commands, reveal unseen state, alter the authoritative world, or enter save, replay, and multiplayer contracts.
- `rendering/terrainPresentation.ts` projects every authoritative terrain kind into a color-independent symbol, world pattern, full rule detail, and compact zone badge. The tactical map renders the badge at a zone edge so terrain remains identifiable without covering the field; construction feedback and Campaign Theatre intelligence retain the complete rule. This is presentation only and may not affect terrain, placement, movement, combat, visibility, saves, replays, or authority.
...
- Each authored Campaign Theatre profile carries a concise opening directive alongside terrain intelligence and the optional Honor objective. The directive is selected content, shown in the Theatre header, and spoken through keyboard focus so a player understands the first intended tactical or civic response before beginning a reign. It must remain presentation and onboarding content: it may not grant an order, modify a scenario, or enter authority, saves, replays, or progression.
- The Tactical Uplink is a five-line battlefield readout: current order/selection, people, labor, stability, and Faith. Every tactical reign begins as a compact `UPLINK [I]` tab carrying the current Mandate progress count, including wide displays; the full report opens only through its header or `I`. A selected battalion reports `H` (remaining defense), `M` (morale), and `S` (supply) as bounded percentages. The same presentation contract drives the persistent defense bar above every battlefield battalion, so tactical attrition can be read at a glance without creating a second source of truth. Durable Civic Record history remains available in the Book of Lessons and end-of-reign report; this preserves vertical room for the state-derived Mandate, threat, and current intel beneath the live status.
...
- Campaign Honors are deterministic evaluations of the completed victorious world but persist only in browser-local campaign presentation data. Crownfall checks for a Regular or better Crown battalion, Rivergate for a surviving Crown Warship, Ashen Oath for eight resolved captives, and Stonewall for an intact opening gate. The selected Campaign Theatre header previews the relevant objective before a reign and announces it through keyboard focus; sealed honors appear as local progress. Honors must never mutate world state, grant a stat bonus, enter saves/replays, or cross the multiplayer authority boundary.
...
- Every Rival Doctrine profile includes a concise player-facing temperament briefing. The Theatre card and keyboard focus pair that qualitative pressure description with the existing authoritative opening-grace and learning values; the copy changes neither value nor any other world rule.
