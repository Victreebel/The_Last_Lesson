...
- The release gate includes browser smoke coverage against the production bundle. Playwright must verify the named application canvas, install manifest, active service worker, loaded WebP tactical art, a nonblank desktop and phone-sized campaign theatre, and no uncaught page errors. This coverage observes presentation only and cannot inject simulation state.
