...
- Captive policy writes a lasting Civic Record instead of a morality meter. Captives taken, integrated, released, and returned through the Crown-only Prisoner Accord remain inspectable facts; an unresolved history of captivity raises rebellion pressure across the empire.
...
- Heirs with inspectable doctrine statements that explain their observed condition, preferred action, intended purpose, confidence, utility decisions, concerns, rewards, punishments, and live lesson feedback. Crown governors can propose provisional lessons through independent action, but only the God-King's feedback gives them confidence.
...
- Fog-respecting Reconnaissance Contacts: left-click an already visible rival battalion, structure, or convoy to inspect its current force or defense condition and terrain consequence in the existing Tactical Uplink. The blue contact focus preserves your Crown selection for an immediate follow-up order and disappears when vision is lost.
...
Every scenario opens with an Imperial Mandate. Its active directive highlights the existing Build, Command, Accord, or Heir control that can advance the objective; this is guidance only, so every order and policy choice remains the player's.
